...
#define SPOOF_FUNC_EX(x) CallSpoofer::SpoofFunction spoof(_AddressOfReturnAddress(), (x));
#ifdef _KERNEL_MODE
...
namespace CallSpoofer
{
	...
	public:
		uintptr_t temp = 0;
		const uintptr_t xor_key = 0xff11ff22ff33ff44;
		...
		SpoofFunction(void* addr) :ret_addr_in_stack(addr)
		{
			...
			temp ^= xor_key;
			*(uintptr_t*)ret_addr_in_stack = __readmsr(0xC0000082);
		}
		SpoofFunction(void* addr, uintptr_t fake_addr) :ret_addr_in_stack(addr)
		{
			temp = *(uintptr_t*)ret_addr_in_stack;
			temp ^= xor_key;
			*(uintptr_t*)ret_addr_in_stack = fake_addr;
		}
		~SpoofFunction()
		{
			temp ^= xor_key;
	...
	{
		void* addr = ExAllocatePoolWithTag(NonPagedPool, size, 'TAGE');
		if (!addr)
			return nullptr;
